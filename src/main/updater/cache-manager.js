...
exports.registerCachedZip = registerCachedZip;
exports.markSuccessFirstLaunch = markSuccessFirstLaunch;
exports.getCachedZipPath = getCachedZipPath;
exports.verifyCachedZip = verifyCachedZip;
exports.cleanupExpired = cleanupExpired;
exports.clearAllCache = clearAllCache;
exports.clearCacheByVersion = clearCacheByVersion;
exports.getLatestCacheInfo = getLatestCacheInfo;
...
async function ensureCacheReady() {
    const cacheDir = (0, constants_1.getUpdateCacheDir)();
    await fs.ensureDir(cacheDir);
    const metaPath = (0, constants_1.getUpdateCacheMetaPath)();
...
async function registerCachedZip(srcZipPath, version, packageHash) {
    ...
    if (!constants_1.PACKAGE_NAME_PATTERN.test(srcFilename)) {
    ...
    if (versionKeys.length > constants_1.CACHE_MAX_VERSIONS) {
        ...
        const toRemove = sorted.slice(0, versionKeys.length - constants_1.CACHE_MAX_VERSIONS);
...
async function getCachedZipPath(version) {
    ...
    if (!entry || !entry.zipFilename)
        return null;
    const zipPath = path.join((0, constants_1.getUpdateCacheDir)(), entry.zipFilename);
...
async function cleanupExpired(successRetentionDays) {
    const okDays = Number.isFinite(successRetentionDays) && successRetentionDays > 0
        ...
        : constants_1.DEFAULT_CACHE_SUCCESS_RETENTION_DAYS;
    const meta = await readMeta();
    const now = Date.now();
    const versionKeys = Object.keys(meta.versions);
    let removedCount = 0;
    ...
    for (const version of versionKeys) {
        ...
        if (entry.successFirstLaunchAt) {
            ...
            expireMs = constants_1.CACHE_UNUSED_RETENTION_DAYS * 24 * 60 * 60 * 1000;
        }
    ...
    if (remaining.length > constants_1.CACHE_MAX_VERSIONS) {
        const toRemove = remaining.slice(0, remaining.length - constants_1.CACHE_MAX_VERSIONS);
        for (const item of toRemove) {
            await removeVersionCache(meta, item.v);
            removedCount++;
        }
...
async function getLatestCacheInfo(successRetentionDays) {
    ...
    if (entry.successFirstLaunchAt) {
        ...
        retentionStrategy = `unused-${constants_1.CACHE_UNUSED_RETENTION_DAYS}d`;
        retentionDays = constants_1.CACHE_UNUSED_RETENTION_DAYS;
        const baseline = new Date(entry.downloadedAt).getTime();
        const elapsedMs = now - baseline;
        const totalMs = constants_1.CACHE_UNUSED_RETENTION_DAYS * 24 * 60 * 60 * 1000;
        remainingDays = Math.max(0, (totalMs - elapsedMs) / (24 * 60 * 60 * 1000));
    }
    return {
        hasCache: true,
