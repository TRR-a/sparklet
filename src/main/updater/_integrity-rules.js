...
exports.INTEGRITY_EXTENSION_BLACKLIST = exports.INTEGRITY_FILENAME_BLACKLIST = void 0;
exports.isExcludedFromIntegrity = isExcludedFromIntegrity;
// ---------- Exact filename blacklist (case-sensitive, full filename) ---------- [精确文件名黑名单 (完整文件名，区分大小写)]
exports.INTEGRITY_FILENAME_BLACKLIST = new Set([
...
exports.INTEGRITY_EXTENSION_BLACKLIST = new Set([
...
function isExcludedFromIntegrity(filename) {
    ...
    if (exports.INTEGRITY_FILENAME_BLACKLIST.has(filename))
        return true;
    // 2) Extension match (lowercase to avoid .DMP vs .dmp leakage) [扩展名匹配 (统一小写，避免 .DMP vs .dmp 漏过滤)]
    const lower = filename.toLowerCase();
    for (const ext of exports.INTEGRITY_EXTENSION_BLACKLIST) {
        if (lower.endsWith(ext))
            return true;
    }
    return false;
}
//# sourceMappingURL=_integrity-rules.js.map